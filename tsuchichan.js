...
client.on('guildMemberAdd', member => {
	const welcomeEmbed = new EmbedBuilder().setColor(0x99FFFF).setDescription(`${member.guild.name}に${member.user}が参加したよ`).setThumbnail(member.user.avatarURL())
...
client.on('guildMemberRemove', member => {
	const goodbyeEmbed = new EmbedBuilder().setColor(0x99FFFF).setDescription(`${member.guild.name}から${member.user}が退出したよ`).setThumbnail(member.user.avatarURL())
...
client.on('voiceStateUpdate', (oldState, newState) => {
	if (oldState.channelId === null && newState.channelId !== null) {
		const connectEmbed = new EmbedBuilder().setColor(0x99FFFF).setDescription(`${newState.channel.name}に${newState.member.user}が参加したよ`).setThumbnail(newState.member.user.avatarURL())
		...
		const disconnectEmbed = new EmbedBuilder().setColor(0x99FFFF).setDescription(`${oldState.channel.name}から${oldState.member.user}が退出したよ`).setThumbnail(oldState.member.user.avatarURL())
