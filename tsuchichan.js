...
client.on('voiceStateUpdate', (oldState, newState) => {
	if (oldState.channelId === null && newState.channelId !== null) {
		const connectEmbed = new EmbedBuilder().setColor(0x99FFFF).setDescription(`${newState.channel.name}に${newState.member.user}が接続したよ`).setThumbnail(newState.member.user.displayAvatarURL())
		...
		const disconnectEmbed = new EmbedBuilder().setColor(0x99FFFF).setDescription(`${oldState.channel.name}から${oldState.member.user}が切断したよ`).setThumbnail(oldState.member.user.displayAvatarURL())
